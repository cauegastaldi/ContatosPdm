package br.edu.ifsp.contatopdm.model
...
class ContactDaoSqlite(context: Context): ContactDao{
    ...
    override fun retrieveContact(id: Int): Contact? {
        ...
        val contact = if (cursor.moveToFirst()) {
            // preencher o contato com dados da posição atual do cursor
            cursor.rowToContact()
        }
        else {
            null
    ...
    override fun deleteContact(contact: Contact) =
        contactSqliteDatabase.delete(CONTACT_TABLE, "$ID_COLUMN = ?", arrayOf(contact.id.toString()))
